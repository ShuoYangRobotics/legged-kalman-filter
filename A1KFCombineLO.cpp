...
void A1KFCombineLO::update_filter(A1SensorData data) {
    ...
    if (sum_contact < 2) {
        ...
        if (mahalanobis_distance < 0.01) {
            ...
            Eigen::Matrix<double, STATE_SIZE, 1> Ky = P01*measurement_jacobian.transpose()*invSy;
            curr_state = x01 - Ky;
            Eigen::Matrix<double, OBSERVATION_SIZE, STATE_SIZE>  invSH = S.fullPivHouseholderQr().solve(measurement_jacobian);
...
void A1KFCombineLO::process(Eigen::Matrix<double, STATE_SIZE, 1> state, 
...
void A1KFCombineLO::measure(Eigen::Matrix<double, STATE_SIZE, 1> state, 
