...
void sensor_callback(const sensor_msgs::Imu::ConstPtr& imu_msg, const sensor_msgs::JointState::ConstPtr& joint_msg) {
  ...
  if (!kf.is_initialized() && first_sensor_received == false) {
  ...
  filterd_joint_msg.name = {"FL0", "FL1", "FL2", "FR0", "FR1",     "FR2",     "RL0",     "RL1",
...
void opti_callback(const geometry_msgs::PoseStamped::ConstPtr& opti_msg) {
  ...
  if (!kf.is_initialized() && first_sensor_received == true) {
  ...
  // if (kf.is_initialized() && data.opti_vel_ready()) {
