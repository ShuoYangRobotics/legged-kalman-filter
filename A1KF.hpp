    ...
    public:
        A1SensorData() {
            for (size_t i = 0; i < 3; ++i) {
                ...
                opti_euler_filter[i] = MovingWindowFilter(1); 

                opti_pos_filter[i] = MovingWindowFilter(5);
                opti_vel_filter_sgolay[i] = gram_sg::SavitzkyGolayFilter(sgolay_order,sgolay_order,sgolay_order,1);
            }
            for (size_t i = 0; i < NUM_DOF; ++i) {
                joint_pos_filter[i] = MovingWindowFilter(2);
                joint_vel_filter[i] = MovingWindowFilter(2);
